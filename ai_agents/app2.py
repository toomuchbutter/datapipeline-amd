    ...
    if not os.path.exists(cache_path):
        return {
            ...
            "capacity_plan": {
                ...
                "confidence": 0.91,
                "future_cpu_forecast": 98.48,
            },
            "hardware_finops": {
                "approved": False,
                "optimized_nodes": 3,
                "hardware_selection": "AMD_INSTINCT_GPU/RIIS_2100",
                "efficiency_justification": "Current EPYC pools non-compliant for intensive burst workloads. Switch to hardware-accelerated matrix cores required.",
...
# ==============================================================================
# 3. NON-THREADED AGNO CLIENT CONVERSION BRIDGE (METHOD 1)
# ==============================================================================
def get_agno_events_sync(payload_msg):
    """Bypasses reverse proxy filters, manually step-advances the Agno async generator

    using a managed loop execution window, yielding events cleanly to Streamlit.
    """
    ...
    try:
        ...
        while True:
            try:
                # Step forward through the async generator manually
...
# ==============================================================================
# 4. INTERACTIVE CHANNELS CHASSIS LAYOUT
# ==============================================================================
...
with tab1:
    ...
    current_health = (
        "SCALING_ACTIVE" if "SCALING" in state.get("system_health") else "HEALTHY"
    )
    ...
    with col_chart_1:
        ...
        if latest_ts not in st.session_state["chart_history"].index:
                ...
                [
                    {
                        ...
                        "Memory_Utilization": float(
                            telemetry.get("memory_avg", 68.38)
                        ),
            ...
            st.session_state["chart_history"] = pd.concat(
                [st.session_state["chart_history"], new_row]
            ).tail(30)

        st.area_chart(
            st.session_state["chart_history"][
                ["vCPU_Utilization", "Safety_Threshold_Limit"]
            ],
            height=320,
        )
        st.caption(
            "🔴 Load Spike Trace Baseline Ceiling at 80%. Shaded Area indicates an intensive traffic spike detected."
        )

    with col_chart_2:
        st.markdown(
            "#### AMD Hardware performance-per-Watt and Cost Efficiency Curve"
        )
        tco_dataframe = pd.DataFrame(
            {
                ...
                "AMD Core Efficiency Score (%)": [
                    91.4 for _ in state.get("timestamps", [1])
                ],
            }
        ).set_index("Timeline")
        ...
        st.caption(
            "🏎️ AMD Efficiency Score Channel Key: Blue = Est. Cost ($/h) | Neon Green = AMD Core Efficiency Score (%)."
        )
...
with tab2:
    ...
    st.caption(
        "Direct interface with Agno Team AgentOS Engine via atomic event lifecycle streaming vectors."
    )
    ...
    if action_btn_placeholder.button(
        "🚀 INITIATE COMPREHENSIVE LIVE EVALUATION SEQUENCE",
        ...
        # Completely Synchronous, Linear Generation. No threads, no queues.
        for event_type, event_data in get_agno_events_sync(prompt_input):

            # A. Evaluate Core Team Run Checkpoints
            if event_type == "TeamRunStarted":
                status_box.info(
                    "🏁 [TeamRunStarted] Orchestration sequence initiated. Synchronizing node matrices..."
                )
                ...
                output_response_box.markdown(
                    f'<div style="background:#090D16; padding:1rem; border-radius:8px; color:#E2E8F0; line-height:1.6;">{full_text_accumulator}🖨️</div>',
                    unsafe_allow_html=True,
                )

            elif event_type == "TeamRunContentCompleted":
                trace_logs_accumulator.append(
                    "📥 [TeamRunContentCompleted] Content stream fully flushed."
                )

            elif event_type == "TeamRunCompleted":
                status_box.success(
                    "🏁 [TeamRunCompleted] Sequence Complete: All multi-agent boardroom evaluation traces captured."
                )
                ...
                error_msg = (
                    str(event_data)
                    if isinstance(event_data, (str, Exception))
                    else getattr(event_data, "error", "Unknown engine failure.")
                )
                status_box.error(
                    f"❌ [TeamRunError] Critical Pipeline Abort: {error_msg}"
                )
                trace_logs_accumulator.append(f"🟥 CRITICAL ERROR: {error_msg}")
                break

            elif event_type == "TeamRunCancelled":
                status_box.warning(
                    "⚠️ [TeamRunCancelled] Execution signal dropped by client framework rules."
                )
                trace_logs_accumulator.append("🟨 Run lifecycle cancelled.")

            # B. Evaluate Injected Structural Core Tool Calls
            elif event_type == "TeamToolCallStarted":
                tool_name = getattr(
                    event_data, "tool_name", "Injected Dependency Tool"
                )
                trace_logs_accumulator.append(
                    f"🛠️ [Tool Started] Calling active routine: `{tool_name}`"
                )

            elif event_type == "TeamToolCallCompleted":
                trace_logs_accumulator.append(
                    "✅ [Tool Completed] Return parameters synchronized successfully."
                )

            # C. Evaluate Complex Multi-Agent Multi-Step Thought Processes
            elif event_type == "TeamReasoningStarted":
                trace_logs_accumulator.append(
                    "🧠 [Reasoning Started] Initiating multi-agent look-ahead calculation trees..."
                )

            elif event_type == "TeamReasoningStep":
                reasoning_chunk = getattr(event_data, "content", "")
                trace_logs_accumulator.append(
                    f" 🔍 [Reasoning Step] {reasoning_chunk.strip()}"
                )

            elif event_type == "TeamReasoningCompleted":
                trace_logs_accumulator.append(
                    "🎯 [Reasoning Completed] Tree resolution finalized."
                )

            # Instantly update the logger terminal code viewport snippet element
            log_terminal.code(
                "\n".join(trace_logs_accumulator), language="markdown"
            )

        # --- AFTER EXECUTION: POST STORAGE UPDATE BACK TO CACHE ---
    ...
    with col_right_grid:
        ...
        st.error(
            f"🛡️ **[Agent 5] Safety Governor Gating:** SAFETY VERDICT: REJECTED | Scaling action throttled due to simulated HIGH risk."
        )
...
with tab3:
    ...
    with col_timeline:
        ...
        if incident_list:
            ...
            st.info(
                "Log parameters clear. Awaiting pipeline execution updates."
            )
    ...
    with col_postmortem:
        ...
        st.markdown(
            safety.get(
                "postmortem_markdown", "No safety governor logs initialized yet."
            )
        )
        postmortems = state.get("postmortems", [])
        for report in reversed(postmortems):
            with st.expander(
                f"📁 Diagnostic Report Timestamp: {report['timestamp']}"
              ):
        ...
        hardware_matrix = pd.DataFrame(
            {
                ...
                "Cluster Power Load (W)": [
                    float(telemetry.get("amd_epyc_power_watts", 339.8))
                    for _ in state.get("timestamps", [1])
                ],
                "AMD EPYC Efficiency Score (%)": [
                    92.4 for _ in state.get("timestamps", [1])
                ],
            }
        ).set_index("Timeline")
        ...
        core_allocation = pd.DataFrame(
            {
                ...
                "AMD_INSTINCT_GPU": [
                    1 if "GPU" in str(finops.get("hardware_selection")) else 0
                ],
            }
        ).set_index("SKU Pool")
